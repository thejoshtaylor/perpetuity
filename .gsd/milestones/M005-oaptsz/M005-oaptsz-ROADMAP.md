...
- [ ] **S05: S05** `risk:Real-device behavior on iOS Safari + Android Chrome may surface UX gaps that mocked tests can't catch (install banner copy on iOS, push permission flow on Android, voice mic UX on small screens). Redaction sweep may surface a leak that requires going back to fix logging in S01–S04.` `depends:[]`
