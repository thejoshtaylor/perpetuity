# S04: Tmux session model + Redis registry + reattach

**Goal:** Verify by citation that M003/S04's success criterion — tmux session model + Redis registry + reattach across orchestrator restart — is already met by code on main shipped under M002/S04 + M002/S05. Produce T01-VERIFICATION.md (criterion-by-criterion citation report with verbatim PASSED lines from cited tests against the live compose stack), re-file the M003-umluob ≡ M002-jy6pde duplication hand-off, and emit zero source/compose/Dockerfile/test-code changes. Mirrors the verification-by-citation slice pattern established by S01/T01 (MEM200/MEM201) and S03/T02 (MEM205).
**Demo:** Integration test (the architectural bet): POST /v1/sessions creates a tmux session. Orchestrator's exec stream pipes 'echo hello\n' into stdin via the WS-style interface (this slice is HTTP-only for now; the WS bridge lands in S05). Wait for output. docker compose restart orchestrator (programmatically). After orchestrator boots and rebuilds state from Redis, GET /v1/sessions/{id}/scrollback returns content containing 'hello'. New exec attach to the same tmux session runs 'echo world' in the same shell.

## Must-Haves

- M003/S04's single demo: POST /v1/sessions creates a tmux session; orchestrator's WS-style exec stream pipes 'echo hello\n' into stdin (slice description's "WS-style interface" — the orchestrator's WS /v1/sessions/{id}/stream is exactly that); wait for output; restart orchestrator; after orchestrator boots and rebuilds state from Redis, scrollback fetch returns content containing 'hello'; new exec attach to the same tmux session runs 'echo world' in the same shell. This is byte-for-byte the demo of `backend/tests/integration/test_m002_s05_full_acceptance_e2e.py::test_m002_s05_full_acceptance` step (a) — DURABILITY. Plus orchestrator-internal proof at `orchestrator/tests/integration/test_ws_bridge.py::test_disconnect_reconnect_preserves_scrollback` that scrollback survives a WS disconnect+reconnect cycle. Plus M002/S04's bundled e2e `test_m002_s04_full_demo` covering multi-tmux + scrollback proxy.

## Proof Level

- This slice proves: - This slice proves: contract (verification-by-citation; no new runtime built)
- Real runtime required: yes (cited tests run against the live compose stack — db + redis + ephemeral orchestrator + workspace image)
- Human/UAT required: no (auto-mode runs the cited tests; the verification artifact is the deliverable)

## Integration Closure

- Upstream surfaces consumed: `orchestrator/orchestrator/sessions.py` (start_tmux_session, capture_scrollback, kill_tmux_session, resize_tmux_session, list_tmux_sessions); `orchestrator/orchestrator/redis_client.py` (RedisSessionRegistry — D013 source-of-truth); `orchestrator/orchestrator/routes_sessions.py` (POST/GET/DELETE/scrollback/resize); `orchestrator/orchestrator/routes_ws.py` (WS /v1/sessions/{id}/stream — the "WS-style interface"); `orchestrator/tests/integration/test_ws_bridge.py::test_disconnect_reconnect_preserves_scrollback`; `backend/tests/integration/test_m002_s04_e2e.py::test_m002_s04_full_demo`; `backend/tests/integration/test_m002_s05_full_acceptance_e2e.py::test_m002_s05_full_acceptance` step (a).
- New wiring introduced in this slice: none — verification + documentation only.
- What remains before the milestone is truly usable end-to-end: a human owner must reconcile the M003-umluob ≡ M002-jy6pde duplication (close M003 as already-delivered, recommended; or `gsd_replan_slice` toward M003's true Projects/GitHub scope per R009–R012). Auto-mode cannot make this call. S05 and S06 are likely also verification-only by the same logic (S05 = WS bridge already shipped under M002/S04 routes_ws.py + test_m002_s04_full_demo; S06 = final integrated acceptance literally is `test_m002_s05_full_acceptance`).

## Verification

- Runtime signals: no new signals introduced. Cited tests exercise existing orchestrator INFO log keys: session_attached, session_detached, container_provisioned, container_reused, session_created, attach_registered, attach_unregistered, image_pull_ok. Existing WARNING keys preserved: tmux_session_orphaned, ws_malformed_frame, docker_exec_stream_error, redis_unreachable.
- Inspection surfaces: T01-VERIFICATION.md is the on-disk durable evidence. `redis-cli -a $REDIS_PASSWORD KEYS 'session:*'` proves the Redis registry is the source-of-truth across restart. `docker exec <ws-container> tmux ls` proves tmux ownership of the pty. `docker compose logs orchestrator` carries the structured log lines the cited tests emit.
- Failure visibility: missing PASSED line, missing `## Criterion:` section, or missing duplication hand-off in T01-VERIFICATION.md is the on-disk failure-state — the slice is not delivered if the report can't be grep-asserted.
- Redaction constraints: same MEM134 leak-safe boundary; verification artifact must not embed user emails or full_names from any test fixture.

## Tasks

- [x] **T01: Verify S04 demo by citation against shipped M002 code; produce T01-VERIFICATION.md** `est:60m`
...
If a cited test fails when re-run on HEAD, surface the failure as a real verification gap — write a `## Verification gap:` section in T01-VERIFICATION.md, include the failing pytest output, and stop the slice. Do NOT modify the test or the orchestrator to make it pass; that's a human-action call (re-plan toward true M003 scope or close M003 as undelivered).
  - Files: `.gsd/milestones/M003-umluob/slices/S04/tasks/T01-VERIFICATION.md`, `.gsd/milestones/M003-umluob/slices/S04/tasks/T01-SUMMARY.md`
