...
**Goal:** Verify the S05 demo (cookie-authed browser WS bridge → backend → orchestrator → tmux: attach frame with scrollback, input/data echo, resize/SIGWINCH, disconnect-race cleanup with tmux survival, and 1008 'session_not_owned' on cross-user attach) by citation against tests already shipped under M002/S04 and M002/S05. Produce T01-VERIFICATION.md with one `## Criterion:` section per S05 sub-criterion, verbatim PASSED lines from live compose-stack runs, file:line citations into backend `sessions.py` proxy and orchestrator `routes_ws.py`/`sessions.py`, and re-file the M003-umluob ≡ M002-jy6pde duplication hand-off (fourth in a row) so a human owner can reconcile M003 before S06.
**Demo:** Integration test: signup creates user + personal team; POST creates a session via the orchestrator; client opens WS to /api/v1/ws/terminal/<session_id> with the auth cookie; receives attach frame with scrollback (empty for fresh session); sends {type: 'input', bytes: 'echo hello\n'}; receives {type: 'data', bytes: 'echo hello\r\nhello\r\n...'}; sends {type: 'resize', cols: 120, rows: 30} — tmux SIGWINCH forwarded. Disconnect race test: send input, immediately close client WS — backend proxy task terminates cleanly, orchestrator-side WS closes, tmux session stays alive (verified by reattach in next test). Ownership test: user B attempts WS to user A's session_id — 1008 close 'session_not_owned'.

## Must-Haves

- T01-VERIFICATION.md exists at `.gsd/milestones/M003-umluob/slices/S05/tasks/T01-VERIFICATION.md` with ≥5 `## Criterion:` sections (cookie auth on browser WS / attach-frame-with-scrollback delivery / input→data echo round-trip / resize SIGWINCH no-error / disconnect race cleanup with tmux alive / cross-owner 1008 'session_not_owned').
- ≥5 verbatim PASSED lines from live test runs against the real compose stack are quoted in T01-VERIFICATION.md (one per criterion minimum).
- File-and-line citations into `backend/app/api/routes/sessions.py` (ws_terminal L354, ownership/auth L373-L405, accept L409, _proxy_frames L478-L535), `orchestrator/orchestrator/routes_ws.py` (session_stream L97-L458, attach frame L146-L173, resize handler L341-L370, attach refcount L228-L240/L453-L458), and `orchestrator/orchestrator/sessions.py` (resize_tmux_session L491-L526) are present.
- Top-level `## Human action required: M003-umluob duplicates M002-jy6pde` block is present (literal grep-stable string `M003-umluob duplicates M002-jy6pde`).
- `git status --porcelain | grep -v '^.. .gsd/'` is empty after the slice runs (zero source/compose/Dockerfile/test code modified).
- Cited tests pass on HEAD: `test_m002_s05_full_acceptance` (DURABILITY + OWNERSHIP), `test_attach_frame_then_echo_roundtrip`, `test_resize_frame_does_not_error`, `test_disconnect_reconnect_preserves_scrollback`, `test_ws_close_emits_attach_unregistered`. Any failure is recorded as a `## Verification gap:` section, not papered over.

## Proof Level

- This slice proves: - This slice proves: integration (verification-by-citation against already-shipped M002/S04 + M002/S05 code; no new functionality).
- Real runtime required: yes — cited tests run against the live compose stack (db + redis + ephemeral orchestrator + workspace image).
- Human/UAT required: yes — the M003-umluob ≡ M002-jy6pde reconciliation hand-off cannot be resolved by auto-mode.

## Integration Closure

- Upstream surfaces consumed (read-only, cited-not-changed):
  - `backend/app/api/routes/sessions.py` ws_terminal L354-L444 (cookie auth via `get_current_user_ws` L373, ownership check L394-L405, accept L409, orchestrator WS dial via two-key auth header pattern, _proxy_frames L478-L535).
  - `backend/app/api/deps.py::get_current_user_ws` (cookie-first WS auth, MEM018/MEM067, pre-accept close on auth failure per MEM022).
  - `orchestrator/orchestrator/routes_ws.py` session_stream L97-L458 (key auth L107-L109, registry lookup L114-L133, attach frame L146-L173, exec stream L175-L226, attach refcount L228-L240/L453-L458, dual pumps L242-L394, teardown L429-L458, resize handler L341-L370).
  - `orchestrator/orchestrator/sessions.py` resize_tmux_session L491-L526 (D017 last-writer-wins SIGWINCH via `tmux refresh-client -C cols,rows`).
  - `orchestrator/orchestrator/attach_map.py` (MEM181 process-local refcount; correctly drops on disconnect).
  - Tests: `test_m002_s05_full_acceptance`, `test_attach_frame_then_echo_roundtrip`, `test_resize_frame_does_not_error`, `test_disconnect_reconnect_preserves_scrollback`, `test_ws_close_emits_attach_unregistered`.
- New wiring introduced in this slice: none. Strict verification-only scope.
- What remains before milestone is truly usable end-to-end: nothing within the M003 success-criteria envelope as currently scoped (the milestone's headline test is the literal `test_m002_s05_full_acceptance` already passing). What DOES remain is the human-action reconciliation: close M003 as already-delivered (recommended) or `gsd_replan_slice` toward R009-R012 Projects/GitHub scope per PROJECT.md. S06 should make the same call independently.

## Verification

- Runtime signals: none added (verification-only). Existing INFO keys exercised by cited tests: `session_proxy_open`, `session_proxy_reject` (sessions.py L401), `attach_registered`, `attach_unregistered`, `ws_malformed_resize`, `tmux_resize_failed`. Existing close codes: 1008 ('session_not_owned'), 1011 ('orchestrator_unavailable'), 1000 (clean exit).
- Inspection surfaces: `docker compose logs backend orchestrator` carries the structured log lines; `docker exec perpetuity-redis-1 redis-cli -a $REDIS_PASSWORD HGETALL session:<id>` shows the registry state across a disconnect; `docker exec <ws-container> tmux ls` proves tmux session survived a WS close; `docker ps --filter label=user_id=… --filter label=team_id=…` lists per-(user,team) containers.
- Failure visibility: T01-VERIFICATION.md is the durable on-disk failure-state — a missing `## Criterion:` section, missing PASSED line, or missing `M003-umluob duplicates M002-jy6pde` hand-off string is the on-disk evidence that the slice is not delivered. The slice-plan grep gate enforces this mechanically. Pre-existing test_ws_bridge `_seed_session` FK seeding bug (MEM209) and linuxkit loop-device-pool flake (MEM210) MUST be recorded as `## Verification gap:` sections rather than masked, if either trips again.

## Tasks

- [x] **T01: Verify S05 demo by citation against shipped M002 code; produce T01-VERIFICATION.md** `est:45m`
...
If a cited test fails when re-run on HEAD, surface the failure as a real verification gap — write a `## Verification gap:` section in T01-VERIFICATION.md, include the failing pytest output, and stop the slice. Do NOT modify the test or the source to make it pass; that's a human-action call (re-plan toward true M003 scope or close M003 as undelivered).
  - Files: `.gsd/milestones/M003-umluob/slices/S05/tasks/T01-VERIFICATION.md`, `.gsd/milestones/M003-umluob/slices/S05/tasks/T01-SUMMARY.md`
