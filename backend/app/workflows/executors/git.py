"""Git step executor: run a git subcommand in the user's workspace.

`run_git_step(session, step_run_id)` drives a single `git` step from
`running` to a terminal state.  The rendered config is expected to contain:

  config = {
      "subcommand": "checkout" | "pull" | "fetch" | "push",
      "args":       [str, ...],  # extra args appended after the subcommand
  }

The executor renders the final argv as `[git, <subcommand>, *args]` and
posts it to the orchestrator one-shot exec endpoint.
...
HTTP transport retries via `_orchestrator_exec_with_retry` (same backoff as
shell executor).

Secret discipline: rendered args may carry substituted form values. Nothing
in config is logged.
"""
...
logger = logging.getLogger("app.workflows.executors.git")

_VALID_SUBCOMMANDS = frozenset({"checkout", "pull", "fetch", "push"})
_DEFAULT_TIMEOUT_SECONDS = 300
_HTTP_TIMEOUT = httpx.Timeout(_DEFAULT_TIMEOUT_SECONDS + 30, connect=5.0)

_SESSION_ID_NAMESPACE = uuid.UUID("c5f2e3a8-3d47-6b4c-b05c-9f6a9e2d1023")
...
def run_git_step(session: Session, step_run_id: uuid.UUID) -> None:
    """Execute a single `git` step.

    Reads the rendered snapshot.config, builds `[git, subcommand, *args]`,
    and drives the step to a terminal state.
    """
    step_run = session.get(StepRun, step_run_id)
    if step_run is None:
        logger.warning("git_step_missing step_run_id=%s", step_run_id)
        return

    workflow_run = session.get(WorkflowRun, step_run.workflow_run_id)
    if workflow_run is None:
        logger.warning("git_step_orphan_run step_run_id=%s", step_run_id)
        return
    ...
    subcommand: str = config.get("subcommand") or ""
    if subcommand not in _VALID_SUBCOMMANDS:
        _mark_failed(
            session,
            step_run,
            error_class="orchestrator_exec_failed",
            stderr=f"invalid git subcommand: {subcommand!r}",
            started_monotonic=time.monotonic(),
        )
        return

    extra_args: list[str] = config.get("args") or []
    ...
    logger.info(
        "step_run_started run_id=%s step_index=%s action=git subcommand=%s",
        step_run.workflow_run_id,
        step_run.step_index,
        subcommand,
    )
